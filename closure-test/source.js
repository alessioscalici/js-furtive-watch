...
var bindInput = function bindInput(obj, prop, elem) {
  
  var eventTarget;
  fw.watch(obj, prop, function (mod) {
    if (eventTarget !== elem)
      elem.value = mod.to;
    else
...
var bindEditable = function bindEditable(obj, prop, elem) {
  
  var eventTarget;
  fw.watch(obj, prop, function (mod) {
    if (eventTarget !== elem)
      elem.innerHTML = mod.to;
    else
      eventTarget = undefined;
  });
  ...
  elem.addEventListener('input', function (ev) {
    eventTarget = ev.target;
    obj[prop] = elem.innerHTML;
  });
};

var bindHtml = function bindHtml(obj, prop, elem) {
  fw.watch(obj, prop, function (mod) {
    elem.innerHTML = mod.to;
  });
};

var bindCheckbox = function bindCheckbox(obj, prop, elem) {
  fw.watch(obj, prop, function (mod) {
    elem.checked = mod.to;
  });
...
bindInput(object, 'name', document.getElementById('pippo'));
bindInput(object, 'name', document.getElementById('pluto'));
bindHtml(object, 'name', document.querySelector('h2.pippo'));
bindCheckbox(object, 'checked', document.getElementById('ck-pippo'));
bindHtml(object, 'checked', document.querySelector('h2.checked'));

bindInput(object, 'number', document.querySelector('input.number'));
bindHtml(object, 'number', document.querySelector('h2.number'));


bindEditable(object, 'name', document.querySelector('h3.contenteditable'));
